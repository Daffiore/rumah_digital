    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-64 bg-[#1e293b] flex flex-col h-full shadow-2xl z-10">
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-2">
                ...
                <p class="text-xs font-bold text-slate-400 mb-2 mt-6 ml-2 tracking-wider">DIVISI  <span class="text-[#0284c7]" >{{ auth()->user()->divisi }}</span></p>
                <div class="pl-4 space-y-1">
                    <a href="/timeline" class="flex items-center gap-3 text-slate-400 hover:text-white text-sm py-2 transition-colors">
                        Timeline Divisi
                    </a>
                    <a href="#" class="flex items-center gap-3 text-slate-400 hover:text-white text-sm py-2 transition-colors">
                        Contact SuperAdmin
                    </a>
                    <a href="/announcements" class="flex items-center gap-3 text-[#0284c7] font-bold text-sm py-2 transition-colors">
                        Announcements
                    </a>
                </div>
            </div>
        ...
        <main class="flex-1 overflow-y-auto bg-gray-50/50 p-6 md:p-10 relative">
            ...
            <div class="mb-8">
                <h1 class="text-3xl font-black text-[#0f172a] uppercase tracking-wider">
                    Buat <span class="text-[#0284c7]">Pengumuman</span>
                </h1>
                <p class="text-gray-500 text-sm mt-1 font-medium">Sampaikan informasi penting kepada seluruh anggota divisi.</p>
            </div>

            <div class="bg-white border border-gray-100 shadow-xl shadow-gray-200/40 rounded-[2rem] p-6 md:p-10 max-w-4xl">
                
                <form action="/announcement/saveannouncement" method="POST">
                    @csrf 
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="title" name="title" value="{{ old('title') }}" 
                                   ...
                                   placeholder="Contoh: Info Rapat Pleno Bulan Ini" required>
                            @error('title') <p class="mt-1 text-xs text-red-500 font-bold">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label for="status" class="block mb-2 text-sm font-bold text-gray-900">Kategori / Prioritas</label>
                            <select id="status" name="status" 
                                ...
                                <option value="" disabled {{ old('status') ? '' : 'selected' }}>Pilih Kategori...</option>
                                <option value="Biasa" {{ old('status') == 'Info' ? 'selected' : '' }}>Info</option>
                                <option value="Penting" {{ old('status') == 'Penting' ? 'selected' : '' }}>Penting</option>
                                <option value="Kegiatan" {{ old('status') == 'Kegiatan' ? 'selected' : '' }}>Kegiatan</option>
                            </select>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="6" 
                                      ...
                                      placeholder="Tuliskan detail pengumuman di sini..." required>{{ old('description') }}</textarea>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 mt-8 pt-6 border-t border-gray-100">
                        ...
                        <a href="/announcements" class="w-full sm:w-auto bg-white hover:bg-gray-50 text-gray-700 border border-gray-200 px-8 py-3 rounded-xl font-bold text-sm transition-all active:scale-95 flex items-center justify-center text-center">
                            Batal
                        </a>
                    </div>
